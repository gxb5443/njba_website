				...
				if ( $attachments )
				{
					foreach ( $attachments as $attachment_id ) {
						echo '<li class="image" data-attachment_id="' . $attachment_id . '">
							...
							<ul class="actions">
								<li><a href="#" class="delete" title="' . __( 'Delete image', 'mm_lang' ) . '">' . __( 'Delete', 'mm_lang' ) . '</a></li>
							</ul>
						</li>';
					}
				}
			...
			jQuery(document).ready(function($){
				...
				jQuery('.add_slider_images').on( 'click', 'a', function( event ) {
					...
					slider_gallery_frame = wp.media.frames.downloadable_file = wp.media({
						...
						title: '<?php _e( 'Add Images to Gallery', 'mm_lang' ); ?>',
						button: {
							text: '<?php _e( 'Add to gallery', 'mm_lang' ); ?>',
						},
						multiple: true
					});
