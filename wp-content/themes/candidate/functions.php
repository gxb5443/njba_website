...
function candidate_theme_setup() {
	...
	register_nav_menus( array(
		'main_navigation' => __( 'Main Navigation', THEMENAME )
	) );
...
if ( ! function_exists( 'candidat_custom_fontello_classes' ) ) {
	function candidat_custom_fontello_classes() {
		...
		$menuIcons = array();
		$menuIcons['none'] = __( '- select icon -', THEMENAME );
		...
		<div class="comment-author">
			...
			<div class="author-meta">
				...
				<div class="comment-meta">
					...
					<span>
					<?php 
					comment_reply_link( array_merge( $args, array('reply_text' => __('Reply', THEMENAME), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); 
					?> 
					</span>
				</div>
			</div>
...
add_filter ( 'wp_tag_cloud', 'candidate_tag_cloud_slug_class' );

if ( ! function_exists( 'candidate_tag_cloud_slug_class' ) ) { 
	function candidate_tag_cloud_slug_class( $taglinks ) {
		$tags = explode('</a>', $taglinks);

		$regex = "#(.*class=')(.*)(' title.*)#e"; 
			foreach( $tags as $tag ) {
			$tag = preg_replace($regex, "('$1tag-size-1  tag $2$3')", $tag );
			 $tagn[] =  $tag;
			}
...
function candidat_widgets_init() {
	...
	register_sidebar( 
		array(
			'id'          =>   'shop',
			'name'        => __( 'shop' , THEMENAME ),
			'before_widget' => '<div id="%1$s" class="sidebar-box white widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3 class="custom_sidebar_title">',
			'after_title'   => '</h3>',
			'description' => __( 'This sidebar is located the page shop.' , THEMENAME ),
		)
	 );
	 ...
	 register_sidebar( 
		array(
			'id'          =>   'blog_default',
			'name'        => __( 'Blog default' , THEMENAME ),
			'before_widget' => '<div id="%1$s" class="sidebar-box white widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3 class="custom_sidebar_title">',
			'after_title'   => '</h3>',
			'description' => __( 'This sidebar is located the default blog.' , THEMENAME ),
		)
	 );
...
if ( ! function_exists( 'candidat_woocommerce_breadcrumbs' ) ) { 
	function candidat_woocommerce_breadcrumbs() {
		return array(
				...
				'before'      => '',
				'after'       => '',
				'home'        => _x( 'Home', 'breadcrumb', THEMENAME ),
			);
	}
}
...
function candidat_the_breadcrumbs() {
    ...
    elseif (is_day()) {echo __( 'Archive: ', THEMENAME ); the_time('F jS, Y'); }
    elseif (is_month()) {echo __( 'Archive: ', THEMENAME ); the_time('F, Y'); }
    elseif (is_year()) {echo __( 'Archive:', THEMENAME ); the_time('Y'); }
    elseif (is_author()) {echo __( 'Author archive: ', THEMENAME ); }
    elseif (isset($_GET['paged']) && !empty($_GET['paged'])) {echo __( 'Blogarchive: ', THEMENAME ); echo'';}
    elseif (is_search()) {echo __( 'Search results: ', THEMENAME ); }
...
////////////tribe_events//////////////////////////////////////////////////////////////////////	
 ...
 if ( ! function_exists( 'candidat_tribe_meta_event_tags2' ) ) {  
	function candidat_tribe_meta_event_tags2( $label=null, $separator=', ', $echo = true ) {
		if ( !$label ) { $label = __( 'Tags', THEMENAME ); }
		...
		case "country" :
			...
			if ( sizeof( $countries ) == 1 ) {
				...
				$field = '<div class="animate-onscroll ' . esc_attr( implode( ' ', $args['class'] ) ) .'" id="' . esc_attr( $key ) . '_field">'
						...
						. '<option value="">'.__( 'Select a country&hellip;', THEMENAME ) .'</option>';
				...
				$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . __( 'Update country', THEMENAME ) . '" /></noscript>';
		...
		case "state" :
			...
			if ( is_array( $states ) && empty( $states ) ) {
				...
				$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" class="chosen-select state_select" ' . implode( ' ', $custom_attributes ) . ' placeholder="' . esc_attr( $args['placeholder'] ) . '">
					<option value="">'.__( 'Select a state&hellip;', THEMENAME ) .'</option>';
