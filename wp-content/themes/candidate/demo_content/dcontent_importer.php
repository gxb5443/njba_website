	...
	if ( current_user_can('manage_options') ) {
		...
		if ( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ) {
			...
			if ( class_exists('UniteFunctionsRev') ) {
				...
				$rev_directory = get_template_directory() . '/demo_content/revolution/';

				foreach( glob( $rev_directory . '*.txt' ) as $filename ) {
					$filename = basename($filename);
					$rev_files[] = get_template_directory_uri() . '/demo_content/revolution/' . $filename ;
				}
